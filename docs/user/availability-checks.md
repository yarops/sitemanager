...
3. **Стратегия уведомлений (Notification strategy)**:
   - **Мгновенно (Immediate)**: Как только сайт перестанет отвечать 200 OK, вы немедленно получите сообщение в Telegram.
   - **Ежедневный отчет (Summary)**: Вы не будете получать алерты по каждой ошибке, но в конце дня придет сводный отчет со списком всех сайтов, где были проблемы.
   - **Отключено**: Проверки проводятся, история сохраняется, но уведомления не отправляются.
